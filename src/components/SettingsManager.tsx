        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {!showForm && (
            <div className="space-y-2 mb-4">
              {labels.map((label) => (
                <div key={label.id} className="flex items-center gap-3 p-3 rounded-xl bg-gray-50">
                  <span
                  ...
                  <button onClick={() => startEdit(label)} className="p-2 rounded-full hover:bg-gray-200">
          ...
          {showForm && (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Short Code (1-4 characters)</label>
                <input
                  type="text"
                  maxLength={4}
          ...
          {!showForm && onGoogleConnect && (
            <div className="mt-6 pt-4 border-t border-gray-200">
              ...
              <p className="text-xs text-gray-500 mb-3">View your Google Calendar events alongside your shifts.</p>
              {googleConnected ? (
                <div className="space-y-3">
                  ...
                  >
                    ...
                    <div
                      className={`w-10 h-6 rounded-full relative transition-colors ${googleVisible ? 'bg-blue-500' : 'bg-gray-300'}`}
                    >
                      <div
                        className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${googleVisible ? 'left-5' : 'left-1'}`}
                      />
                    </div>
                  </button>
                  <button
          ...
          <div className="absolute z-20 inset-0 flex items-center justify-center p-6">
            <div className="bg-white rounded-xl shadow-lg p-5 w-full max-w-xs">
              ...
              <p className="text-sm text-gray-600 mb-4">
                Are you sure you want to delete <strong>{deletingLabel.name}</strong>? This will also remove it from any
                assigned days.
              </p>
              <div className="flex gap-2">
                <button
